 ...
 *
 * This is the client-side installer script. It should be hosted on your server
 * and included on your users' websites.
 *
...
(function () {
    ...
    // ===================================================================================
    const YOUR_SERVER_URL = "http://localhost:3000"; // <-- ❗ CHANGE THIS ❗
    // ===================================================================================

    const host = window.location.host;
    const COOKIE_NAME = "myPushModalSeen"; // Renamed to avoid conflicts

    /**
     * Initializes Firebase Messaging, requests permission, and sends the token to our server.
     * This function is attached to the window object so the UI widgets can call it.
     * @param {object} firebaseConfig - Your public Firebase config.
        ...
        window.myPushRequestAndRegister = function () {
            ...
            Notification.requestPermission().then((permission) => {
                if (permission === 'granted') {
                    ...
                    // Update UI to show permission is blocked
    ...
    // --- SCRIPT LOADER ---
    // Dynamically loads Firebase scripts in sequence.
    ...
    firebaseAppScript.onload = function () {
        ...
        messagingScript.onload = function () {
            // --- MAIN EXECUTION ---
            // Now that Firebase scripts are loaded, fetch config from OUR server.
            ...
            .then(config => {
                ...
                // Your public Firebase config and VAPID key
                ...
                // Prepare the Firebase messaging system
                initializeFirebaseMessaging(myFirebaseConfig, myVapidKey);

                // Initialize UI widgets if they are configured
                if (config.roll_services && typeof config.roll_services === 'object') {
                    initRoll(config.roll_services);
                }

                if (config.flask_services && typeof config.flask_services === 'object') {
                    initModal(config.flask_services);
                }
            })
    ...
    // ===================================================================================
    //  UI WIDGET CODE
    //  The following code is adapted from the aplu.io script for UI rendering.
    // ===================================================================================
    ...
    let updateSubUI; // Declare globally within the IIFE
    ...
    const initRoll = (cfg) => {
        ...
        //  Instead of directly calling Notification.requestPermission, we call our
        //  global function that also handles sending the token to our server.
        // =========================================================================
        subscribeBtn.addEventListener("click", () => {
            if (window.myPushRequestAndRegister) {
                // Optimistically update the UI. The function will handle the rest.
    ...
    // --- You can paste the FlaskBox Modal code (initModal, injectFlaskBox) here if you plan to use it ---
    // For brevity, it has been omitted, but the structure is the same. Just add it here.
